...
// noise factors
float noiseScale = 450, noiseStrength = 50;

// noise display factors
float overlayAlpha = 100, irisAlpha = 255, strokeWidth = .35;
...
class Iris {
  ...
  void reDrawIt() {
    // new noise
    noiseScale = int(random(450, 755));
    noiseStrength = int(random(30, 55));
    noiseDetail(int(random(1, 15)), noise(.55)); //explore second number as varialble to make things nuts

    // parameters reset
    x = sx;
    y = sy;
    NDo = int(random(50, 360));
    sx = width/2  + radius * cos(NDo);
    sy = height/2 + radius * sin(NDo);
    ox = x;
    oy = y;
